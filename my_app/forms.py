...
class ProfileForm(FlaskForm):
    ...
    company = SelectField('Управляющая компания', coerce=int, validators=[InputRequired()])
...
class PriceForm(FlaskForm):
    ...
    maintenance_common = FloatField("Содержание жилья (руб/м.кв)", validators=[Optional()])
    drainage_common = FloatField("Водоотведение (общее) (руб/м.куб)", validators=[Optional()])
    cold_water_common = FloatField("ХВС (общее) (руб/м.куб)", validators=[Optional()])
    hot_water_volume_common = FloatField("ГВС теплоноситель (общее) (руб/м.куб)", validators=[Optional()])
    hot_water_energy_common = FloatField("ГВС тепловая энергия (общее) (руб/Гкал)", validators=[Optional()])
    electricity_common = FloatField("Электроэнергия (общее) (руб/кВт.ч)", validators=[Optional()])
    heating = FloatField("Отопление (руб/Гкал)", validators=[Optional()])
    cold_water = FloatField("ХВС (руб/м.куб)", validators=[Optional()])
    hot_water_volume = FloatField("ГВС теплоноситель (руб/м.куб)", validators=[Optional()])
    hot_water_energy = FloatField("ГВС тепловая энергия (руб/Гкал)", validators=[Optional()])
